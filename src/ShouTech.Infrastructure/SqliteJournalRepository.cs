...
public sealed class SqliteJournalRepository : IJournalRepository
{
    ...
    public SqliteJournalRepository(string connectionString)
    ...
    public async Task SaveDraftAsync(Guid companyId, JournalEntry entry, string userId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
                ...
                """, connection, transaction))
            {
                ...
                command.Parameters.AddWithValue("@company_id", companyId.ToString());
                command.Parameters.AddWithValue("@entry_date", entry.EntryDate.ToString("yyyy-MM-dd"));
            ...
            foreach (var line in entry.Lines)
            {
                await using var command = new SqliteCommand("""
                    ...
                    """, connection, transaction);
                command.Parameters.AddWithValue("@entry_id", entry.Id.ToString());
                command.Parameters.AddWithValue("@line_number", lineNumber++);
                command.Parameters.AddWithValue("@account_id", line.AccountId.ToString());
    ...
    private async Task ChangeStatusAsync(Guid companyId, Guid id, string userId, bool reverse, string? details, CancellationToken ct)
    {
        ...
        try
        {
            await using var check = new SqliteCommand(
                "SELECT status FROM journal_entries WHERE journal_entry_id=@id AND company_id=@company_id",
                connection, transaction);
            check.Parameters.AddWithValue("@id", id.ToString());
            check.Parameters.AddWithValue("@company_id", companyId.ToString());
            ...
            if (!reverse)
            {
                await using var balance = new SqliteCommand("""
                    SELECT CASE
                        WHEN COUNT(*) >= 2
                         AND COALESCE(SUM(debit), 0) = COALESCE(SUM(credit), 0)
                         AND COALESCE(SUM(debit), 0) > 0
                        THEN 1 ELSE 0 END
                    FROM journal_entry_lines
                    WHERE journal_entry_id=@id
                    """, connection, transaction);
                balance.Parameters.AddWithValue("@id", id.ToString());
                if (Convert.ToInt32(await balance.ExecuteScalarAsync(ct).ConfigureAwait(false), System.Globalization.CultureInfo.InvariantCulture) != 1)
                    throw new InvalidOperationException("Journal entry is not balanced or has no lines.");
            }

            await using var update = new SqliteCommand(reverse
                ? "UPDATE journal_entries SET status=2, reversed_by=@user_id, reversed_at=CURRENT_TIMESTAMP WHERE journal_entry_id=@id AND company_id=@company_id"
                : "UPDATE journal_entries SET status=1, posted_by=@user_id, posted_at=CURRENT_TIMESTAMP WHERE journal_entry_id=@id AND company_id=@company_id",
                connection, transaction);
            update.Parameters.AddWithValue("@id", id.ToString());
            update.Parameters.AddWithValue("@company_id", companyId.ToString());
            update.Parameters.AddWithValue("@user_id", userId.Trim());
            await update.ExecuteNonQueryAsync(ct).ConfigureAwait(false);

            await using var audit = new SqliteCommand("""
